...
@section('title', 'កែមុខវិជ្ជា - Edit Course')
...
<style>
    ...
    .ac-page-icon { width: 52px; height: 52px; border-radius: 16px; background: linear-gradient(135deg, #e65100, #ef6c00); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; box-shadow: 0 6px 18px rgba(230,81,0,0.28); flex-shrink: 0; }
    ...
    .step-badge { width: 26px; height: 26px; border-radius: 50%; background: #e65100; color: #fff; font-size: 0.72rem; font-weight: 800; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    ...
    .ac-meta-card { background: linear-gradient(135deg,#fff8e1,#ffecb3); border-radius: 16px; padding: 1.25rem 1.4rem; border: 1px solid rgba(230,81,0,0.15); }
    .ac-meta-card h6 { font-size: 0.82rem; font-weight: 700; color: #e65100; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 7px; }
    .ac-meta-list { list-style: none; padding: 0; margin: 0; }
    .ac-meta-list li { font-size: 0.78rem; color: #6b7280; line-height: 1.8; display: flex; gap: 7px; }
    .ac-meta-list li strong { color: #374151; }
...
@php
    $courseDays = $course->days ? explode(',', $course->days) : [];
@endphp
...
<div class="ac-wrap">
    ...
    <div class="ac-page-header">
        <div class="ac-page-header-left">
            ...
            <div>
                ...
                <h1 class="ac-page-title">កែមុខវិជ្ជា</h1>
                <div class="ac-page-sub">Edit course: {{ $course->name }}</div>
            </div>
        </div>
    ...
    <div class="ac-layout">
        ...
        <form action="{{ route('courses.update', $course->id) }}" method="POST" id="courseForm">
            ...
            {{-- Hidden: color + icon --}}
            <input type="hidden" name="color" id="hiddenColor" value="{{ old('color', $course->color ?? '#1a237e') }}"/>
            <input type="hidden" name="icon"  id="hiddenIcon"  value="{{ old('icon',  $course->icon  ?? '💻') }}"/>
            ...
            <div class="ac-card">
                ...
                <div class="ac-card-body">
                    ...
                    <div class="ac-field" style="margin-bottom:1rem;">
                        ...
                        <div class="ac-char-count"><span id="nc">{{ strlen(old('name', $course->name ?? '')) }}</span>/80</div>
                        ...
                        <div class="ac-field">
                            ...
                            <div class="ac-input-icon-wrap">
                                ...
                                <input type="text" name="code" id="courseCode"
                                    ...
                                    placeholder="WEB-101" maxlength="20"
                                    oninput="liveUpdate()" style="text-transform:uppercase"/>
                            </div>
                        ...
                        <div class="ac-field">
                            ...
                            <select name="category" id="courseCategory"
                                ...
                                @foreach(['Web Development','Database','Networking','UI/UX Design','Graphic Design','Cybersecurity','AI & ML','Mobile App','IT Support','Digital Marketing','Algorithm','OS Concepts'] as $c)
                                    <option value="{{ $c }}" {{ old('category', $course->category) == $c ? 'selected':'' }}>{{ $c }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="ac-field">
                        ...
                        <textarea name="description" id="courseDesc"
                            ...
                            oninput="liveUpdate(); countChar(this,'dc',300)">{{ old('description', $course->description) }}</textarea>
                        <div class="ac-char-count"><span id="dc">{{ strlen(old('description', $course->description ?? '')) }}</span>/300</div>
                            ...
                            <div class="ac-input-icon-wrap">
                                ...
                                <select name="teacher" class="ac-select @error('teacher') is-invalid @enderror" style="padding-left:34px;" onchange="liveUpdate()">
                                    ...
                                    @foreach(['លោក លីហួ','អ្នកគ្រូ សុភា','លោក ចាន់ត្រា','លោក ហេង បូរ៉ា','អ្នកគ្រូ ម៉ែន ច័ន្ទ','លោក វង់ ពីរោះ'] as $t)
                                        <option value="{{ $t }}" {{ old('teacher', $course->teacher) == $t ? 'selected':'' }}>{{ $t }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="ac-field">
                            ...
                            <div class="ac-dur-row">
                                <input type="number" name="duration" id="courseDuration" class="ac-input"
                                    ...
                                    placeholder="0" min="1" oninput="liveUpdate()"/>
                        ...
                        <div class="ac-field">
                            ...
                            <div class="ac-input-icon-wrap">
                                ...
                                <input type="number" name="max_students" id="maxStudents" class="ac-input"
                                    ...
                                    placeholder="30" min="1" oninput="liveUpdate()"/>
                            </div>
                        </div>
                    </div>
                            ...
                            @foreach(['ច','អ','ពុ','ព្រ','សុ','ស'] as $d)
                                <label class="day-check">
                                    <input type="checkbox" name="days[]" value="{{ $d }}"
                                        {{ in_array($d, old('days', $courseDays)) ? 'checked':'' }}>{{ $d }}
                                </label>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="ac-field">
                            ...
                            <div class="ac-input-icon-wrap">
                                ...
                                <input type="time" name="time_start" id="timeStart" class="ac-input"
                                    value="{{ old('time_start', $course->time_start) }}"
                                    oninput="liveUpdate()"/>
                            </div>
                        </div>
                        <div class="ac-field">
                            ...
                            <div class="ac-input-icon-wrap">
                                ...
                                <input type="time" name="time_end" id="timeEnd" class="ac-input"
                                    value="{{ old('time_end', $course->time_end) }}"
                                    oninput="liveUpdate()"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ac-field">
                        ...
                        <div class="ac-input-icon-wrap">
                            ...
                            <input type="text" name="room" id="courseRoom" class="ac-input"
                                ...
                                placeholder="e.g. A101, Lab-B" oninput="liveUpdate()"/>
                        </div>
                    </div>
                    ...
                    <div class="ac-field" style="margin-bottom:1.1rem;">
                        ...
                        <div class="ac-color-row">
                            ...
                            @foreach($colors as $clr)
                                <div class="ac-color-swatch {{ old('color', $course->color ?? '#1a237e') == $clr ? 'selected':'' }}"
                    ...
                    <div class="ac-field">
                        ...
                        <div class="ac-icon-grid">
                            ...
                            @foreach($icons as $ico)
                                <div class="ac-icon-opt {{ old('icon', $course->icon ?? '💻') == $ico ? 'selected':'' }}"
                                     data-icon="{{ $ico }}"
        ...
        <div class="ac-preview-col">
            <div class="ac-preview-card">
                <div class="ac-preview-bar" id="pBar" style="background:{{ $course->color ?? '#1a237e' }};"></div>
                <div class="ac-preview-head">
                    <span>Live Preview</span>
                    <span class="ac-preview-live">LIVE</span>
                </div>
                <div class="ac-preview-body">
                    <div class="ac-prev-icon" id="pIcon" style="background:{{ $course->color ?? '#1a237e' }};">{{ $course->icon ?? '💻' }}</div>
                    <div class="ac-prev-cat"  id="pCat"  style="color:{{ $course->color ?? '#1a237e' }};">{{ $course->category ?? 'ប្រភេទ' }}</div>
                    <div class="ac-prev-name" id="pName">{{ $course->name ?? 'ឈ្មោះមុខវិជ្ជា' }}</div>
                    <div class="ac-prev-code" id="pCode">{{ $course->code ?? 'CODE-000' }}</div>
                    <div class="ac-prev-desc" id="pDesc">{{ $course->description ?? 'ការពិពណ៌នា...' }}</div>
                    <div class="ac-prev-chips">
                        <span class="ac-prev-chip" id="pLevel">🎯 {{ $course->level ?? 'Level' }}</span>
                        @if($course->time_start || $course->time_end)
                        <span class="ac-prev-chip" id="pTime">🕐 {{ $course->time_start ?? '--' }}–{{ $course->time_end ?? '--' }}</span>
                        @else
                        <span class="ac-prev-chip" id="pTime" style="display:none">🕐</span>
                        @endif
                        @if($course->room)
                        <span class="ac-prev-chip" id="pRoom">🏫 {{ $course->room }}</span>
                        @else
                        <span class="ac-prev-chip" id="pRoom" style="display:none">🏫</span>
                        @endif
                        @if($course->max_students)
                        <span class="ac-prev-chip" id="pMax">👥 {{ $course->max_students }}</span>
                        @else
            ...
            {{-- Meta info --}}
            <div class="ac-meta-card">
                <h6>📋 ព័ត៌មានដើម</h6>
                <ul class="ac-meta-list">
                    <li><strong>ID:</strong> #{{ $course->id }}</li>
                    <li><strong>Code:</strong> {{ $course->code }}</li>
                    <li><strong>បានបង្កើត:</strong> {{ \Carbon\Carbon::parse($course->created_at)->format('d M Y') }}</li>
                    <li><strong>កែចុងក្រោយ:</strong> {{ \Carbon\Carbon::parse($course->updated_at)->format('d M Y') }}</li>
                </ul>
            </div>
        </div>
...
<script>
let selColor = '{{ old('color', $course->color ?? '#1a237e') }}';
let selIcon  = '{{ old('icon',  $course->icon  ?? '💻') }}';
