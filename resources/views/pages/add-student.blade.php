...
@section('title', 'បន្ថែមសិស្ស - Add Student')

@section('content')
<style>
    .as-wrap { max-width: 780px; margin: 0 auto; padding: 2.5rem 2rem 4rem; }
    ...
    .as-icon { width: 52px; height: 52px; border-radius: 16px; background: linear-gradient(135deg,#1a237e,#3949ab); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; box-shadow: 0 6px 18px rgba(26,35,126,0.25); }
    ...
    .as-row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem; }
    @media(max-width:580px){ .as-row-2 { grid-template-columns: 1fr; } }
    ...
    .as-input::placeholder { color: #9ca3af; }
    .as-input:focus, .as-select:focus { outline: none; border-color: #1a237e; box-shadow: 0 0 0 3px rgba(26,35,126,0.1); background: #fff; }
    ...
    .gender-lbl:hover { border-color: #1a237e; color: #1a237e; }
    .gender-opt:checked + .gender-lbl { background: #1a237e; border-color: #1a237e; color: #fff; }
    ...
    .score-bar-fill { height: 100%; border-radius: 4px; background: #1a237e; width: 0; transition: width 0.3s, background 0.3s; }
    ...
    <div class="as-header">
        <div class="as-header-left">
            ...
            <div>
                ...
                <h1 class="as-title">បន្ថែមសិស្សថ្មី</h1>
                <div class="as-sub">Add a new student to the school system</div>
            </div>
        </div>
            ...
            <div class="as-card-body">
                    ...
                    <div class="as-field">
                        ...
                        <div class="iw">
                            ...
                            <input type="number" name="age" class="as-input @error('age') is-invalid @enderror"
                                value="{{ old('age') }}" placeholder="e.g. 18" min="5" max="100"/>
                        </div>
                ...
                <div class="as-field">
                    ...
                    <div class="gender-row">
                        <input type="radio" name="gender" id="g_male"   class="gender-opt" value="male"   {{ old('gender')=='male'   ? 'checked':'' }}>
                        <label for="g_male"   class="gender-lbl">👦 ប្រុស</label>

                        <input type="radio" name="gender" id="g_female" class="gender-opt" value="female" {{ old('gender')=='female' ? 'checked':'' }}>
                        <label for="g_female" class="gender-lbl">👧 ស្រី</label>

                        <input type="radio" name="gender" id="g_other"  class="gender-opt" value="other"  {{ old('gender')=='other'  ? 'checked':'' }}>
                ...
                <div class="as-field">
                    ...
                    <div class="score-preview">
                        <div class="score-bar-wrap">
                            <div class="score-bar-fill" id="scoreBar" style="width:{{ old('score',0) }}%;"></div>
                        </div>
                        <span class="score-lbl" id="scoreLabel" style="color:#1a237e;">
                            {{ old('score') ? old('score').'%' : '0%' }}
                        </span>
                    </div>
                </div>
...
<script>
    ...
    // Init if old value
    const initScore = document.getElementById('scoreInput').value;
    if (initScore) updateScore(initScore);
</script>
